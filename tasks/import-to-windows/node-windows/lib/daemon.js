   ...
   *     });
      ...
      value: function(dir) {
        ...
        if (this.exists) {
          ...
          if (!fs.existsSync(path.join(this.root, this.id + '.xml'))) {
            this.log.warn('The primary configuration file is missing or cannot be found ('
    ...
    start: {
      ...
      value: function() {
        ...
        this.execute('NET START "' + me.name + '"', function(err, stdout, stderr) {
          if (err) {
            if (err.code == 2) {
              if (err.message.indexOf('already been started') >= 0 && err.message.indexOf('service name is invalid')
                < 0) {
    ...
    exists: {
      ...
      get: function() {
        ...
        return fs.existsSync(path.join(this.directory(), this.id + '.exe'))
