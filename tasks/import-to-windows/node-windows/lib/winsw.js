module.exports = {
  ...
  generateXml: function(config) {
    ...
    if (config.dependencies) {
      config.dependencies = (config.dependencies instanceof Array
      == true) ? config.dependencies : config.dependencies.split(',');
  ...
  createExe: function(name, dir, callback) {
    ...
    var origin = p.join(__dirname, '..', 'bin', 'winsw', 'x' + (require('os').arch().indexOf('64')
        > 0 ? '64' : '86'), 'winsw.exe'),
